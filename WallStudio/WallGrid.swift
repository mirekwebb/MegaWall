...
func queryWallpapers() {
    ...
    if byLikes { query.order(byDescending: WALLPAPERS_LIKES)
    } else { query.order(byDescending: "createdAt") }
...
@IBAction func optionsButt(_ sender: Any) {
    ...
    if wallObj[WALLPAPERS_FAVORITED_BY] != nil {
        ...
        } else { addFavActionTitle = "+ Add to Favorites" }
    ...
    if wallObj[WALLPAPERS_LIKED_BY] != nil {
        ...
        } else { likeActionTitle = "Like this wallpaper" }
    ...
    if isFavorites { alert.addAction(removeFav)
    } else { alert.addAction(addFav) }
    alert.addAction(cancel)
...
@IBAction func shareButt(_ sender: AnyObject) {
    let messageStr = "Hey, check out this wallpaper found on #\(APP_NAME)!"
    let image = imgPrev.image!

    let shareItems:Array = [messageStr, image] as [Any]
